...
5. Получаем данные из выгрузок, в которых есть .csv, и переходим к их обработке - назначем каждому столбцу выгрузки отдельную переменную и формируем дополнительные переменные из полученных данных: 
...
6. Исходя из полученных данных, выгрузки делятся по нескольким параметрам на:
...
7. После определения процесса переходим к изменениям данных в AD:
...
- Проверка, существует ли пользователь:
  ...
  - Если не находим пользователя, то записываем информацию об этом в лог файл
   ----
8. В зависимоти от того, в какой из блоков попала выгрузка после отработки блока могут производится следующие действия:
...
9. В ходе выполнения всего скрипта для каждой выгрузки собираются критические ошибки, наличие которых говорит о том , что один из блоков или функций неккоректно отработали:
...
10. Отдельно обработаны выгрузки, в которых нет csv файла - только фото. В нём информацию по сотруднику ($User_UID) получаем из нейминга фото и по нему производим поиск учётной записи в AD сотрудника.
